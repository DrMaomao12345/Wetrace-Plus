...
> 仓库里也**没有**提取密钥、解密数据库或本地加密文件的代码（v1.0.4 起连继承来的 `.dat` 解码器一并删除）。
> 唯一的例外是表情包：导出数据里带着微信 CDN 地址和消息自带的 key，显示时按它解包，且只允许访问微信自己的 CDN。

---

## 安全

- 默认只监听 `127.0.0.1`。**从其他设备访问必须先设置密码或创建移动端配对** —— 没有凭据的远程请求一律 401。
- 通过域名（比如 Tailscale MagicDNS）访问时，把域名写进 `.env` 的 `ALLOWED_HOSTS`；
  其余域名一律拒绝，用来挡 DNS 重绑定。
- 浏览器发起的跨站写请求会被拒绝；移动端配对只能「看」和「算」，改不了设置。
- 发现漏洞请看 [SECURITY.md](SECURITY.md)，不要公开提 issue。

## 法律与许可

- **只做导入与分析。** Wetrace Plus 不连接微信、不读取微信进程、不提取任何密钥，也不解密任何数据库；
  仓库里没有相关代码（v1.0.4 起，连继承来的 `.dat` 图片解码器也一并删掉了）。数据来自你用别的工具
  导出的文件。
- **许可证：[AGPL-3.0](LICENSE)。** 可以自由使用、修改和再分发；如果你改了它并对外提供服务，
  按 AGPL 的要求需要把源码一并公开。
- **署名：** 部分基础代码派生自以 Apache-2.0 发布的
  [chatlog](https://github.com/sjzar/chatlog)，相关文件保留原作者版权头；
  署名与改动说明见 [NOTICE](NOTICE)。
- **请只处理你有权处理的聊天记录**，并遵守所在地法律与相关服务条款。导出文件、分析库和备份都属于
  高度敏感数据，安全由使用者自己负责。本项目按「现状」提供，不附任何担保。
...
Wetrace Plus 的分析框架与同源项目 chatlog 一脉相承，并移除了其中的微信进程访问、密钥获取、
数据库解密与源目录同步能力 —— 剩下的只有「读导出文件、算统计、画图」。
...
本仓库以 [AGPL-3.0](LICENSE) 发布；继承自 chatlog 的部分按 Apache-2.0 署名（见 [NOTICE](NOTICE)），第三方依赖各自遵循其许可证。
