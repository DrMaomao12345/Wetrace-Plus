...
func (s *Service) setupRoutes() {
	...
	v1 := s.router.Group("/api/v1")
	{
		...
		aiGroup := v1.Group("/ai")
		{
			...
			aiGroup.GET("/memories/:talker", s.api.GetContactMemory)
			aiGroup.POST("/memories/:talker/rebuild", s.api.RebuildContactMemory)
			aiGroup.DELETE("/memories/:talker", s.api.DeleteContactMemory)
		...
		analysisGroup := v1.Group("/analysis")
		{
			...
			analysisGroup.GET("/forecast/:id", s.api.GetChatForecast)
